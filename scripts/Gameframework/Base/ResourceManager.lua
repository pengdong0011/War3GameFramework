...
-- 物品技能
local itemAbilityIntID = {}

-- 建立名称与物编ID的对应关系
local unitName2ID = {}
local abilityName2ID = {}
local itemName2ID = {}
...
---@return string
function mt.GetIDByIndex(header, index)
    Yuyuko.LinqList.Clear(t)
    index = index - 1
...
function mt.IsItemId(intID)
    if not itemAbilityIntID[intID] then
        return false
    else
        return true
    end
...
function mt.GetUnitIDByName(name) return unitName2ID[name] end

function mt.GetAbilityIDByName(name) return abilityName2ID[name] end

function mt.GetItemIDByName(name) return itemName2ID[name] end

function mt.GetUnitSlk(unitID, property, default)
    local unit_data = slk.unit[unitID]
    if not unit_data then
        Yuyuko.Log.error('单位数据未找到', unitID)
        return default
    end
    local data = unit_data[property]
    if data == nil then return default end
    if type(default) == 'number' then return tonumber(data) or default end
    return data
end

function mt.GetAbilitySlk(abilityID,property,default)
    local ability_data = slk.ability[abilityID]
    if not ability_data then
        Yuyuko.Log.error('技能数据未找到', abilityID)
        return default
    end
    local data = ability_data[property]
    if data == nil then return default end
    if type(default) == 'number' then return tonumber(data) or default end
    return data
end
